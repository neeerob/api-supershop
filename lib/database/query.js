    ...
    db: {
      collection(collection) {
        return {
          ...
          async updateOne(key, data, viewC) {
            try {
              let view = {};
              if (viewC) {
                view = viewC;
              ...
              // Create a filter based on the provided key
              const filter = { _id: ObjectId(key) };

              // Use $set to update the existing document with newObj
              const updateExp = await app
                .listeners(dbCall)[0](collection, "updateOne", filter, {
                  $set: newObj,
                })
                .then(async () => {
                  let result = await find().findById.bind({
                    collection,
                    key,
                    view,
                  })();
